...
===============
*/
void G_CheckSniperCall ( weapon_t weapon, gentity_t* attacker, gentity_t* target, bool miss )
{
	vec3_t diff;
	...
	if ( VectorLengthSquared ( diff ) > 1500 * 1500 )
	{
		...
		if ( miss )
		{
			if ( nearby )
			{
				G_VoiceGlobal ( target, "sniper", false );
			}
		}
		// Look for someone around the guy being shot at to say sniper
		else if ( nearby )
		{
			G_VoiceGlobal ( nearby, "sniper", false );
		}
	}
}
...
===============
*/
void G_TraceBullet ( weapon_t weapon, trace_t* tr, G2Trace_t G2Trace, vec3_t start, vec3_t end, int passent, int mask, bool detailed )
{
	int			unlinkCount;
	...
	while ( 1 )
	{
		...
		if ( detailed && tr->fraction != 1 && g_entities[ tr->entityNum ].client )
		{
			...
			G_CheckSniperCall ( weapon, &g_entities[passent], traceEnt, G2Trace[0].mEntityNum != traceEnt->s.number ? true : false );
...
void G_FireBullet ( gentity_t* ent, int weapon, int attack )
{
	...
	detailed = (attackDat->pellets > 1 || attackDat->melee) ? false : true;
...
gentity_t* G_FireProjectile ( gentity_t *ent, weapon_t weapon, attackType_t attack, int projectileLifetime, int flags )
{
	...
	if ( weapon >= WP_M67_GRENADE && weapon < WP_M15_GRENADE && (flags&PROJECTILE_TIMED) && (ent->client->ps.pm_type == PM_NORMAL) )
	{
		...
		if ( nearby )
		{
			G_VoiceGlobal ( ent, "fire_hole", true );
		}
	}
