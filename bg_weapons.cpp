...
bool BG_InitAmmoStats(void)
{
	...
	GP2 = trap_GP_ParseFile("ext_data/sof2.ammo", true, false);
	if (!GP2)
	{
		return false;
	}
...
static bool BG_ParseAttackStats ( int weaponNum, attackData_t* attack, void *attacksub )
{
	void*	sub;
	char	tmpStr[256];
...
static bool BG_ParseWeaponStats(weapon_t weaponNum, void *group)
{
	char		 tmpStr[256];
	weaponData_t *weapon;
...
static bool BG_ParseWeapon(weapon_t weapon, void *group)
{
...
Returns true if the given weapon has ammo for its alternate attack
===============
*/
bool BG_WeaponHasAlternateAmmo ( int weapon )
{
