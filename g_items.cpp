...
int Pickup_Weapon (gentity_t *ent, gentity_t *other, bool* autoswitch ) 
{
	int			quantity;
...
gentity_t *LaunchItem( gitem_t *item, vec3_t origin, vec3_t velocity ) 
{
	...
	{
		dropped = G_SpawnGametypeItem ( item->pickup_name, true );
		dropped->nextthink = 0;
	}
	else
	{
...
=================
*/
bool G_IsAmmoBeingShared ( gentity_t* ent, int ammoIndex, weapon_t exclude )
{
	// Need to figure out if this guy has any other guns that use the ammo for the gun
	// being dropped.
	for (int weapon = WP_KNIFE + 1; weapon < WP_NUM_WEAPONS; weapon ++ )
	{
...
gentity_t* G_DropWeapon ( gentity_t* ent, weapon_t weapon, int pickupDelay )
{
	...
	ent->client->noOutfittingChange = true;
